package com.spms.constants;

public class RedisConstants {

    public static final String USER_LOGIN = "login:user:";
    public static final Long USER_LOGIN_TTL = 30L;
    public static final String USER_LOGIN_FAIL = "login:fail:";
    public static final Long USER_LOGIN_FAIL_TTL = 10L;

    public static final String EMAIL_CODE = "email:code:";
    public static final Long EMAIL_CODE_TTL = 5L;

    public static final String ROLE_LIST = "role:list:";
    public static final String MENU_LIST = "menu:list";

    public static final String UPDATE_AVATAR = "updateAvatar";
    public static final Long UPDATEAVATAR_TTL = 60L;

    public static final String NOTIFICATION_KEY = "notification:";

    public static final String RECENT_VISIT_PRO = "recent:visit:pro:";
    public static final String RECENT_VISIT_OTHER = "recent:visit:other:";

}
